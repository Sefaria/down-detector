...
- **Deploy is driven by `scripts/web-entrypoint.sh`.** The web container is the single migrator: on every deploy it runs `migrate` (fatal) → `collectstatic` (non-fatal) → `check --deploy` (informational) → `gunicorn`. The scheduler/cleanup containers reuse the image but override the command and never migrate; they gate on the DB's health and only *order* after web. The container `HEALTHCHECK` hits `/healthz`, and production always appends `localhost`/`127.0.0.1` to `ALLOWED_HOSTS` so that loopback probe passes regardless of operator config — don't remove that, it's what keeps deploys from failing as "web unhealthy".
