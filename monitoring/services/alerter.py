...
def send_alert(
    ...
    Args:
        ...
        outage: The Outage DB record associated with this transition
    ...
    try:
        ...
        if transition == "went_down":
            blocks = _build_down_alert(result, outage)
            text = f"🔴 Service Down: {result.service_name}"
        elif transition == "recovered":
            blocks = _build_recovery_alert(result, outage)
...
def _build_down_alert(result: "HealthCheckResult", outage: "Outage | None" = None) -> list[dict]:
    """Build Block Kit blocks for a service down alert."""
    status_page_url = getattr(settings, "STATUS_PAGE_URL", "https://status.sefaria.org")
    
    if outage:
        timestamp = outage.start_time.strftime("%Y-%m-%d %H:%M:%S UTC")
    else:
...
def _get_downtime_duration(outage: "Outage | None" = None) -> str:
    """
    Calculate how long a service was down. Uses the Outage record.
...
def _build_recovery_alert(
    result: "HealthCheckResult", outage: "Outage | None" = None
) -> list[dict]:
    """Build Block Kit blocks for a service recovery alert."""
    status_page_url = getattr(settings, "STATUS_PAGE_URL", "https://status.sefaria.org")
    timestamp = timezone.now().strftime("%Y-%m-%d %H:%M:%S UTC")
    downtime = _get_downtime_duration(outage)
...
def process_transitions_with_alerts(
    transitions: list[tuple["HealthCheckResult", str, "Outage | None"]]
) -> int:
    """
    Process a list of transitions and send alerts for each.
    
    Args:
        transitions: List of (result, transition_type, outage) tuples
    ...
    for result, transition, outage in transitions:
        if send_alert(result, transition, outage):
            alerts_sent += 1
